...
//  WritBot
//
//  Created by Macbook 5 on 3/16/23.
//

import UIKit
@objc(AKDropDown)
open class AKDropDown: UITextField {
    var arrow: Arrow!
...
//        table.appBorderCorner()
        ...
        if parentController?.view.subviews.contains(shadow) == false {
            
        }
        if parentController?.view.subviews.contains(table) == false {
...
extension AKDropDown: UITextFieldDelegate {
...
extension AKDropDown: UITableViewDataSource {
...
extension AKDropDown: UITableViewDelegate {
...
extension UIView {
    func dropShadowList(scale: Bool = true) {
