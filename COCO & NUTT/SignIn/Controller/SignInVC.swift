...
class SignInVC: UIViewController {
    ...
    lazy var iconImage:UIImageView = {
     ...
     lazy var tableView:UITableView = {
        ...
        tv.delegate = self
        tv.dataSource = self
        tv.register(SigninTableCell.self, forCellReuseIdentifier: SigninTableCell.identifier)
...
extension SignInVC:UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier:SigninTableCell.identifier, for: indexPath) as? SigninTableCell {
            cell.logInButton.addTarget(self, action: #selector(buttonTapped), for: .touchUpInside)
            return cell
        }
